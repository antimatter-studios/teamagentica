...
import (
	...
	"github.com/antimatter-studios/teamagentica/plugins/infra-cost-explorer/internal/storage"
)
...
func (h *Handler) HandleUsageEvent(c *gin.Context) {
	...
	if err := json.Unmarshal([]byte(envelope.Detail), &report); err != nil {
		log.Printf("infra-cost-explorer: failed to parse usage detail from %s: %v", envelope.PluginID, err)
	...
	if err := h.db.Insert(rec); err != nil {
		log.Printf("infra-cost-explorer: failed to store usage from %s: %v", envelope.PluginID, err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "storage failed"})
		return
	}

	log.Printf("infra-cost-explorer: stored usage from %s provider=%s model=%s", envelope.PluginID, report.Provider, report.Model)
