...
func TestMatchRoute_SingleMatch(t *testing.T) {
	routes := []Route{
		{PluginID: "messaging-telegram", Prefix: "/messaging-telegram", TargetHost: "host1", TargetPort: 8443},
	}
	matched := MatchRoute(routes, "/messaging-telegram/webhook")
	if matched == nil {
		t.Fatal("expected a match")
	}
	if matched.PluginID != "messaging-telegram" {
		t.Errorf("expected messaging-telegram, got %q", matched.PluginID)
	}
}
...
func TestMatchRoute_LongestPrefixWins(t *testing.T) {
	routes := []Route{
		...
		{PluginID: "long", Prefix: "/messaging-telegram", TargetHost: "host2", TargetPort: 8443},
	}
	matched := MatchRoute(routes, "/messaging-telegram/webhook")
	if matched == nil {
		t.Fatal("expected a match")
...
func TestBuildRemainingPath_Empty(t *testing.T) {
	result := BuildRemainingPath("/messaging-telegram", "/messaging-telegram")
	if result != "/" {
...
func TestBuildRemainingPath_WithSubpath(t *testing.T) {
	result := BuildRemainingPath("/messaging-telegram/webhook", "/messaging-telegram")
	if result != "/webhook" {
...
func TestBuildTargetURL_Simple(t *testing.T) {
	...
	expected := "https://localhost:8443/webhook"
	if result != expected {
...
func TestBuildTargetURL_WithQuery(t *testing.T) {
	...
	expected := "https://host:9000/path?foo=bar&baz=qux"
	if result != expected {
		t.Errorf("expected %q, got %q", expected, result)
	}
}
...
func TestNormalizePrefix_AlreadyNormalized(t *testing.T) {
	result := NormalizePrefix("/messaging-telegram")
	if result != "/messaging-telegram" {
		t.Errorf("expected '/messaging-telegram', got %q", result)
	}
}
