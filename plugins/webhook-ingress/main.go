...
import (
	...
	"github.com/antimatter-studios/teamagentica/plugins/webhook-ingress/internal/config"
)
...
func main() {
	...
	sdkClient.OnEvent("plugin:registered", pluginsdk.NewNullDebouncer(func(event pluginsdk.EventCallback) {
		...
		broadcastWebhookReady(sdkClient, hostname, cfg.HTTPPort)
	}))

	// Start SDK (registration + heartbeat + event subscriptions).
