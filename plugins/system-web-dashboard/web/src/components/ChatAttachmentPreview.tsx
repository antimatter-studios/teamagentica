...
export default function ChatAttachmentPreview({ attachment, compact }: Props) {
  ...
  return (
    <Suspense fallback={<div className="p-2 text-xs text-muted-foreground">Loading...</div>}>
      {compact ? (
        <div className="inline-flex h-24 w-24 overflow-hidden rounded-md border bg-muted/30">
          <Preview src={src} filename={filename} contentType={mimeType} className="h-full w-full" />
        </div>
      ) : (
