...
func discoverTools(sdk *pluginsdk.Client) []discoveredTool {
	...
	if err != nil {
		log.Printf("agent-google: tool discovery failed: %v", err)
		return nil
	}
	...
	for _, p := range plugins {
		...
		if err != nil {
			log.Printf("agent-google: failed to get tools from %s: %v", p.ID, err)
			continue
		}
		...
		if err := json.Unmarshal(body, &resp); err != nil {
			log.Printf("agent-google: failed to parse tools from %s: %v", p.ID, err)
			continue
		}
	...
	if len(allTools) > 0 {
		...
		log.Printf("agent-google: discovered %d tools: %s", len(allTools), strings.Join(names, ", "))
	}
