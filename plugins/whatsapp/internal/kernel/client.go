...
// FindAIAgent discovers an AI chat plugin via capability search.
func (c *Client) FindAIAgent() (string, error) {
	...
	if resp.StatusCode != http.StatusOK {
		return "", fmt.Errorf("kernel search returned %d: %s", resp.StatusCode, string(body))
	}
...
func (c *Client) ChatWithAgent(chatID, message string, imageURLs []string) (string, error) {
	pluginID, err := c.FindAIAgent()
	if err != nil {
		return "", fmt.Errorf("finding AI agent: %w", err)
	}

	return c.chatWithPlugin(chatID, pluginID, "", message, imageURLs, "")
}
