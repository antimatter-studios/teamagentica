...
func discoverTools(sdk *pluginsdk.Client) []discoveredTool {
	...
	if err != nil {
		log.Printf("agent-ollama: tool discovery failed: %v", err)
		return nil
	}
	...
	for _, p := range plugins {
		...
		if err != nil {
			log.Printf("agent-ollama: failed to get tools from %s: %v", p.ID, err)
			continue
		}
		...
		if err := json.Unmarshal(body, &resp); err != nil {
			log.Printf("agent-ollama: failed to parse tools from %s: %v", p.ID, err)
			continue
		}
	...
	if len(allTools) > 0 {
		...
		log.Printf("agent-ollama: discovered %d tools: %s", len(allTools), strings.Join(names, ", "))
	}

	return allTools
}

func buildToolDefs(tools []discoveredTool) []ollama.ToolDef {
	defs := make([]ollama.ToolDef, len(tools))
	for i, t := range tools {
		defs[i] = ollama.ToolDef{
			Type: "function",
...
func executeToolCall(sdk *pluginsdk.Client, tools []discoveredTool, call ollama.ToolCall) (string, error) {
...
func processToolResultMedia(result string) (string, []mediaAttachment) {
	...
	if hasImage && hasMime && imageData != "" && mimeType != "" {
		attachments = append(attachments, mediaAttachment{MimeType: mimeType, ImageData: imageData})
	...
	if rawAtts, ok := parsed["attachments"].([]interface{}); ok {
		for _, rawAtt := range rawAtts {
			...
			if attURL != "" && attMime != "" {
				attachments = append(attachments, mediaAttachment{MimeType: attMime, Type: attType, URL: attURL, Filename: attFilename})
			}
		}
