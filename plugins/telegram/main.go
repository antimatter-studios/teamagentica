...
import (
	...
	"github.com/antimatter-studios/teamagentica/plugins/telegram/internal/bot"
	"github.com/antimatter-studios/teamagentica/plugins/telegram/internal/config"
...
func main() {
	...
	kernelClient := kernel.NewClient(cfg.KernelBaseURL(), cfg.ServiceToken, cfg.Debug)
	allowedUsers := cfg.ParseAllowedUsers()
	telegramBot, err := bot.New(rootCtx, cfg.TelegramToken, kernelClient, cfg.PluginID,
		allowedUsers, cfg.PollTimeout, cfg.Debug, aliases, cfg.DefaultAgent)
	if err != nil {
	...
	sdkClient.OnEvent("webhook:ready", pluginsdk.NewNullDebouncer(func(event pluginsdk.EventCallback) {
		...
		sendRouteUpdate(sdkClient, cfg.PluginID, hostname, cfg.HTTPPort)
	}))

	// When webhook-ingress sends us our full webhook URL, switch to webhook mode.
...
// sendRouteUpdate sends an addressed webhook:api:update event to webhook-ingress.
func sendRouteUpdate(sdkClient *pluginsdk.Client, pluginID, hostname string, port int) {
	...
	log.Printf("Sent webhook:api:update to webhook-ingress: prefix=/%s target=%s:%d", pluginID, hostname, port)
}
