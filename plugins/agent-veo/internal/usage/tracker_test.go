...
func TestRecordAndRetrieve(t *testing.T) {
	...
	if records[0].Model != "veo-3.1-generate-preview" {
		t.Errorf("expected model veo-3.1-generate-preview, got %s", records[0].Model)
	}
	if records[0].TaskID != "veo-1" {
		t.Errorf("expected task_id veo-1, got %s", records[0].TaskID)
	}
...
func TestSummary(t *testing.T) {
	...
	if models["veo-3.1-generate-preview"] != 2 {
		t.Errorf("expected veo-3.1 count 2, got %d", models["veo-3.1-generate-preview"])
	}
}
