...
func newTestHandler(apiKey, model string) *Handler {
	return NewHandler(apiKey, model, "/tmp/test-veo", false)
}
...
func TestHealth(t *testing.T) {
	...
	if resp["plugin"] != "agent-veo" {
		t.Errorf("expected plugin=agent-veo, got %v", resp["plugin"])
	}
	...
	if resp["model"] != "veo-3.1-generate-preview" {
		t.Errorf("expected model=veo-3.1-generate-preview, got %v", resp["model"])
	}
	if resp["status"] != "ok" {
...
func TestHealthNoAPIKey(t *testing.T) {
	h := newTestHandler("", "veo-3.1-generate-preview")
	w := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(w)
...
func TestGenerateNoAPIKey(t *testing.T) {
	...
	body := `{"prompt": "a sunset timelapse"}`
...
func TestConfigOptionsModel(t *testing.T) {
	...
	w := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(w)
	c.Params = gin.Params{{Key: "field", Value: "VEO_MODEL"}}
