...
func Start(ctx context.Context, cfg Config) error {
	...
	log.Printf("[sss3-storage] sss3 subprocess started (pid %d, port %d)", cmd.Process.Pid, cfg.Port)

	// Monitor process exit in background
	go func() {
		if err := cmd.Wait(); err != nil && ctx.Err() == nil {
			log.Printf("[sss3-storage] sss3 exited unexpectedly: %v", err)
			os.Exit(1)
		}
	}()
	...
	log.Printf("[sss3-storage] sss3 is ready on port %d", cfg.Port)
	return nil
}
