...
// DownloadMedia retrieves the download URL for a media object by its ID.
func (c *Client) DownloadMedia(mediaID string) (string, error) {
	url := fmt.Sprintf("%s/%s", graphAPIBase, mediaID)

	if c.debug {
		log.Printf("[whatsapp] GET %s", url)
	}
	...
	if c.debug {
		log.Printf("[whatsapp] media response status=%d body=%s", resp.StatusCode, string(respBody))
	}

	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return "", fmt.Errorf("media API returned %d: %s", resp.StatusCode, string(respBody))
	}
	...
	if err := json.Unmarshal(respBody, &result); err != nil {
		return "", fmt.Errorf("unmarshal media response: %w", err)
	}
	if result.URL == "" {
		return "", fmt.Errorf("no url in media response: %s", string(respBody))
	}
...
func (c *Client) SendImage(to, imageURL, caption string) error {
...
// SendVideo sends a video message with an optional caption.
func (c *Client) SendVideo(to, videoURL, caption string) error {
	video := map[string]string{"link": videoURL}
	if caption != "" {
...
func (c *Client) SendAudio(to, audioURL string) error {
	payload := map[string]interface{}{
		"messaging_product": "whatsapp",
		"to":                to,
		"type":              "audio",
