...
func (h *Handler) ChatStream(ctx context.Context, req pluginsdk.AgentChatRequest) <-chan pluginsdk.AgentStreamEvent {
	...
	go func() {
		...
		if apiKey == "" {
			ch <- pluginsdk.StreamError("No API key configured. Set REQUESTY_API_KEY.")
			return
		}
		...
		stream := requesty.ChatCompletionStream(ctx, apiKey, model, messages)
		...
		for ev := range stream {
			if ev.Err != nil {
				...
				h.emitEvent("error", fmt.Sprintf("requesty stream: %v", ev.Err))
				ch <- pluginsdk.StreamError("Requesty stream error: " + ev.Err.Error())
				return
			}
...
// convertMessages converts SDK conversation messages to requesty messages.
func convertMessages(req pluginsdk.AgentChatRequest) []requesty.Message {
	if len(req.Conversation) > 0 {
		msgs := make([]requesty.Message, len(req.Conversation))
		for i, m := range req.Conversation {
			msgs[i] = requesty.Message{Role: m.Role, Content: m.Content}
		}
		return msgs
	}
	return []requesty.Message{{Role: "user", Content: req.Message}}
}
