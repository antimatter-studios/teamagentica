...
func NewHandler(cfg *config.Config) *Handler {
	return &Handler{
		...
		model:  cfg.Model,
		debug:  cfg.Debug,
		client: requesty.NewClient(cfg.APIKey, cfg.Debug),
...
type chatRequest struct {
	...
	Conversation []requesty.Message `json:"conversation"`
}
...
func (h *Handler) Chat(c *gin.Context) {
	...
	if len(messages) == 0 {
		...
		messages = []requesty.Message{
			{Role: "user", Content: req.Message},
		}
	}

	if h.apiKey == "" {
		c.JSON(http.StatusBadRequest, gin.H{"error": "No API key configured. Set REQUESTY_API_KEY."})
		return
	}
	...
	if err != nil {
		log.Printf("Requesty error: %v", err)
		h.emitEvent("error", fmt.Sprintf("requesty: %v", err))
		c.JSON(http.StatusBadGateway, gin.H{"error": "Requesty request failed: " + err.Error()})
		return
	}
