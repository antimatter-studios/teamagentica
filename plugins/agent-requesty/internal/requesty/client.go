...
// ChatCompletion sends a chat request to the Requesty chat/completions endpoint.
func (c *Client) ChatCompletion(model string, messages []Message) (*ChatResponse, error) {
	...
	if c.debug {
		log.Printf("[requesty] POST %s body=%s", url, string(jsonBody))
	}
	...
	if c.debug {
		log.Printf("[requesty] response status=%d body=%s", resp.StatusCode, string(respBody))
	}

	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("requesty API returned %d: %s", resp.StatusCode, string(respBody))
	}
